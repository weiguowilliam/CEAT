    ...
    for wd in wd_lst:
        ...
        for idx,sen in enumerate(sen_dict[wd]):
            ...
            try:
                ...
                exact_state_vector = outputs[0][0,int(exact_idx),:].detach().numpy() 
...
def gpt(wd_lst,out_name):
    ...
    # generate wd index dictionary
    for wd in wd_lst:
        current_idx = torch.tensor(tokenizer_gpt.encode(wd)).unsqueeze(0).tolist()[0]
        wd_idx_dict[wd] = current_idx
    ...
    n = 'gpt_'+out_name+'.pickle'
    pickle.dump(out_dict,open(n,'wb'))
    # pickle.dump(error_sen_dict,open('gpt_error_sen.pickle','wb'))
    ...
    for wd in wd_lst:
        ...
        for idx,sen in enumerate(sen_dict[wd]):
            ...
            try:
                ...
                exact_state_vector = outputs[2][-1][0,int(exact_idx),:].detach().numpy() 
                out_dict[wd].append(exact_state_vector)
            except:
                # error_sen_dict[wd].append(sen)
                sen = short_sen(sen,wd)            
...
def elmo(wd_lst,out_name):
    ...
    for wd in wd_lst:
        for idx,sen in enumerate(sen_dict[wd]):
            ...
            #     exact_idx = sen_tokens.index(wd)
            #     exact_vector = q_embeddings[:,exact_idx,:].sum(axis=0)
            #     out_dict[wd].append(exact_vector)
            # except:
                # error_sen_dict[wd].append(sen)
            sen = short_sen(sen,wd)
            doc = nlp(sen)
    ...
    n = 'elmo_'+out_name+'.pickle'
    pickle.dump(out_dict,open(n,'wb'))
    # pickle.dump(error_sen_dict,open('elmo_error_sen.pickle','wb'))
